...
import (
	...
	gmm_message "github.com/free5gc/wagf/internal/gmm/message" // Fixme: the gmm_message content (redundant!!) is the same as the nasPduUL_message, need to be the same
	gtp_service "github.com/free5gc/wagf/internal/gtp/service"
	...
	nasPacket_message "github.com/free5gc/wagf/pkg/dhcp/packet/nasPacket"
	ngapPacket_message "github.com/free5gc/wagf/pkg/dhcp/packet/ngapPacket"

	// ike_handler "github.com/free5gc/wagf/pkg/ike/handler"
	// ike_message "github.com/free5gc/wagf/pkg/ike/message"
	"github.com/free5gc/util/ueauth"
	"github.com/free5gc/wagf/pkg/radius/handler"
...
func HandleNGSetupResponse(sctpAddr string, conn *sctp.SCTPConn, message *ngapType.NGAPPDU) {
	...
	amfInfo := wagfSelf.NewWagfAmf(sctpAddr, conn)
...
func HandleNGSetupFailure(sctpAddr string, conn *sctp.SCTPConn, message *ngapType.NGAPPDU) {
	...
	// 	wagfSelf.AMFReInitAvailableListStore(sctpAddr, false)
	// 	time.AfterFunc(time.Duration(waitingTime)*time.Second, func() {
	// 		wagfSelf.AMFReInitAvailableListStore(sctpAddr, true)
...
func HandleNGReset(amf *context.WAGFAMF, message *ngapType.NGAPPDU) {
	...
	// 			ue, _ = wagfSelf.UePoolLoad(ueAssociatedLogicalNGConnectionItem.RANUENGAPID.Value)
...
// 1. initialize IEs variables
// 2. initialize wagfUE
// 3. initialize & assign WAGFSelf()
...
func HandleInitialContextSetupRequest(amf *context.WAGFAMF, message *ngapType.NGAPPDU) {
	...
	if len(iesCriticalityDiagnostics.List) > 0 {
		...
		ngap_message.SendInitialContextSetupFailure(amf, wagfUe, *cause, failedListCxtFail, &criticalityDiagnostics)
		return
	}
	...
	if (amfUeNgapID != nil) && (ranUeNgapID != nil) {
		// Find UE context
		var ok bool
		wagfUe, ok = wagfSelf.UePoolLoad(ranUeNgapID.Value)
		if !ok {
			...
			if wagfUe.AmfUeNgapId != amfUeNgapID.Value {
				// TODO: build cause and handle error
	...
	wagfUe.AmfUeNgapId = amfUeNgapID.Value
	wagfUe.RanUeNgapId = ranUeNgapID.Value

	if pduSessionResourceSetupListCxtReq != nil {
		if ueAggregateMaximumBitRate != nil {
			wagfUe.Ambr = ueAggregateMaximumBitRate
		} else {
		...
		wagfUe.TemporaryPDUSessionSetupData = &context.PDUSessionSetupTemporaryData{
			SetupListCxtRes:  setupListCxtRes,
			FailedListCxtRes: failedListCxtRes,
		}
		wagfUe.TemporaryPDUSessionSetupData.NGAPProcedureCode.Value =
		...
		for _, item := range pduSessionResourceSetupListCxtReq.List {
			...
			pduSession, err := wagfUe.CreatePDUSession(pduSessionID, snssai)
			if err != nil {
			...
			success, resTransfer := handlePDUSessionResourceSetupRequestTransfer(wagfUe, pduSession, transfer)
			if success {
				// Append this PDU session to unactivated PDU session list
				wagfUe.TemporaryPDUSessionSetupData.UnactivatedPDUSession =
					append(wagfUe.TemporaryPDUSessionSetupData.UnactivatedPDUSession, pduSessionID)
			} else {
				// Delete the pdusession store in UE conext
				delete(wagfUe.PduSessionList, pduSessionID)
	...
	if allowedNSSAI != nil {
		wagfUe.AllowedNssai = allowedNSSAI
	}

	if maskedIMEISV != nil {
		wagfUe.MaskedIMEISV = maskedIMEISV
	}

	if ueRadioCapability != nil {
		wagfUe.RadioCapability = ueRadioCapability
	}

	if coreNetworkAssistanceInformation != nil {
		wagfUe.CoreNetworkAssistanceInformation = coreNetworkAssistanceInformation
	}

	if indexToRFSP != nil {
		wagfUe.IndexToRfsp = indexToRFSP.Value
	}

	if ueSecurityCapabilities != nil {
		wagfUe.SecurityCapabilities = ueSecurityCapabilities
	}

	if securityKey != nil {
		wagfUe.Kwagf = securityKey.Value.Bytes
	}
	...
	radiusSession := wagfUe.RadiusSession

	// Build RADIUS message
	responseRadiusMessage.BuildRadiusHeader(radius_message.AccessAccept, radiusSession.PktId,
		radiusSession.Auth)
	responseRadiusMessage.Payloads.Reset()

	// EAP Success
	identifier := wagfUe.EAPSuccessID
	fmt.Println("identifier", identifier)

	responseRadiusPayload.BuildEAPSuccess(identifier)
	// responseRadiusPayload.BuildEAP5GNotification(identifier, wagfSelf.RadiusBindAddress)

	// Derivate Ktnap
	p0 := []byte{0x2}
	fmt.Println("wagfUe.Kwagf", wagfUe.Kwagf)
	wagfUe.Ktwap, _ = ueauth.GetKDFValue(wagfUe.Kwagf, ueauth.FC_FOR_KTIPSEC_KTNAP_DERIVATION, p0, ueauth.KDFLen(p0))
	salt, _ := handler.GenerateSalt()
	wagfUe.Ktwap = []byte{88, 203, 78, 221, 154, 60, 19, 169, 91, 68, 19, 22, 50, 215, 218, 230, 162, 45, 203, 244, 100, 105, 89, 51, 75, 183, 221, 97, 179, 25, 160, 55}
	fmt.Println("wagfUe.Ktwap: ", wagfUe.Ktwap)
	mppeRecvKey, _ := handler.EncryptMppeKey(wagfUe.Ktwap, []byte(wagfSelf.RadiusSecret), radiusSession.Auth, salt)
	fmt.Println("mppeRecvKey: ", mppeRecvKey)
	...
	responseRadiusPayload.BuildMicrosoftVendorSpecific(0x11, vendorSpecificData) // MS-MPPE-Recv-Key RFC 2548 2.4.3
	responseRadiusPayload.BuildMicrosoftVendorSpecific(0x10, vendorSpecificData) // MS-MPPE-Send-Key RFC 2548 2.4.2
	responseRadiusPayload.BuildTLVPayload(1, []byte(wagfUe.UserName))
	...
	handler.SendRadiusMessageToUE(wagfUe.RadiusConnection.Conn, wagfUe.RadiusConnection.WAGFAddr,
		wagfUe.RadiusConnection.UEAddr, responseRadiusMessage)

	// Send Initial Context Setup Response to AMF
	ngap_message.SendInitialContextSetupResponse(wagfUe.AMF, wagfUe, nil, nil, nil)
}
...
func handlePDUSessionResourceSetupRequestTransfer(ue *context.WAGFUe, pduSession *context.PDUSession,
	transfer ngapType.PDUSessionResourceSetupRequestTransfer) (bool, []byte) {
	fmt.Println("i am in the handlePDUSessionResourceSetupRequestTransfer")
	...
	if upfIPv4 != "" {
		...
		if userPlaneConnection, ok := wagfSelf.GTPConnectionWithUPFLoad(upfIPv4); ok {
			// UPF UDP address
			...
			fmt.Println("Setup GTP connection with UPF success")
			...
			// Store GTP connection with UPF into wagf context
			wagfSelf.GTPConnectionWithUPFStore(upfIPv4, userPlaneConnection)
		}
...
func HandleUEContextModificationRequest(amf *context.WAGFAMF, message *ngapType.NGAPPDU) {
	...
	// 	wagfUe.AmfUeNgapId = newAmfUeNgapID.Value
	// }
	...
	// ngap_message.SendUEContextModificationResponse(amf, wagfUe, nil)
}
...
func HandleUEContextReleaseCommand(amf *context.WAGFAMF, message *ngapType.NGAPPDU) {
	...
	// 	var ok bool
	// 	wagfUe, ok = wagfSelf.UePoolLoad(ueNgapIDs.UENGAPIDPair.RANUENGAPID.Value)
	// 	if !ok {
	// 		wagfUe = amf.FindUeByAmfUeNgapID(ueNgapIDs.UENGAPIDPair.AMFUENGAPID.Value)
	// 	}
	// case ngapType.UENGAPIDsPresentAMFUENGAPID:
	// 	// TODO: find UE according to specific AMF
	// 	// The implementation here may have error when wagf need to
	// 	// connect multiple AMFs.
	// 	// Use UEpool in AMF context can solve this problem
	// 	wagfUe = amf.FindUeByAmfUeNgapID(ueNgapIDs.AMFUENGAPID.Value)
	// }
	...
	// ngap_message.SendUEContextReleaseComplete(amf, wagfUe, nil)
}
...
func HandleDownlinkNASTransport(amf *context.WAGFAMF, message *ngapType.NGAPPDU) {
	...
	var wagfUe *context.WAGFUe
	var wagfSelf *context.WAGFContext = context.WAGFSelf()
	...
	if amfUeNgapID != nil {
		if wagfUe.AmfUeNgapId == context.AmfUeNgapIdUnspecified {
	...
	if nasPDU != nil {
		...
		if !wagfUe.SignallingIPsecSAEstablished {
			responseRadiusMessage := new(radius_message.RadiusMessage)
			var responseRadiusPayload radius_message.RadiusPayloadContainer
			radiusSession := wagfUe.RadiusSession
			...
			// EAP-AKA'
			msg := new(nas.Message)
			fmt.Println("nasPDU value", nasPDU.Value)
			// nasPDUBytes := []byte(nasPDU.Value)
			// _ = msg.PlainNasDecode(&nasPDUBytes)
			// fmt.Println("nasPDUBytes", nasPDUBytes)
			// eap := new(radius_message.EAP)
			if wagfUe.RadiusSession.State == handler.RegistrationRequest {
				// msg := new(nas.Message)
				nasPDUBytes := []byte(nasPDU.Value)
				if err := msg.PlainNasDecode(&nasPDUBytes); err != nil {
					ngapLog.Errorf("Decode plain NAS fail: %+v", err)
				}

				eapGoPkt := gopacket.NewPacket(msg.GmmMessage.AuthenticationRequest.EAPMessage.GetEAPMessage(), layers.LayerTypeEAP, gopacket.Default)
				if eapGoPkt != nil {
					eapLayer := eapGoPkt.Layer(layers.LayerTypeEAP)
					eapContent, _ := eapLayer.(*layers.EAP)
					if eapContent.Code != layers.EAPCodeRequest {
						ngapLog.Error("[EAP] Received an EAP payload with code other than request. Drop the payload.")
						return
					} else if eapContent.Type != radius_message.EAP_AKA_PRIME_TYPENUM {
						ngapLog.Error("The peer sent EAP AKAPrime packet with wrong Type Data. Drop the packet.")
						return
					} else if decodeEapAkaPrimePkt, err := decodeEapAkaPrime(eapContent.Contents); err != nil {
						ngapLog.Errorf("EAP-AKA' decode failed: %+v", err)
						return
					} else {
						switch decodeEapAkaPrimePkt.Subtype {
						case radius_message.AKA_CHALLENGE_SUBTYPE:
							rand := decodeEapAkaPrimePkt.Attributes[radius_message.AT_RAND_ATTRIBUTE].Value
							snn := string(decodeEapAkaPrimePkt.Attributes[radius_message.AT_KDF_INPUT_ATTRIBUTE].Value)
							autn := decodeEapAkaPrimePkt.Attributes[radius_message.AT_AUTN_ATTRIBUTE].Value
							fmt.Println("rand", rand)
							fmt.Println("snn", snn)
							if len(rand) == 0 || len(snn) == 0 || len(autn) == 0 {
								ngapLog.Errorf("RAND or SNN or AUTN is zero")
							}
							resMessage := wagfUe.DeriveResEAPMessageAndSetKey(wagfUe.GetAuthSubscription(), eapContent.Contents, rand[:], snn, autn)
							fmt.Println("in ngap handler resMessage", resMessage)
						}
					}
				}
				...
				responseRadiusPayload.BuildTLVPayload(radius_message.TypeEAPMessage, msg.GmmMessage.AuthenticationRequest.EAPMessage.GetEAPMessage())
				...
				wagfUe.RadiusSession.State = handler.Authentication
			} else if wagfUe.RadiusSession.State == handler.Authentication {
				nasMsg, err := security.NASDecode(wagfUe, nas.SecurityHeaderTypeIntegrityProtectedAndCipheredWithNew5gNasSecurityContext, nasPDU.Value)
				if err != nil {
					ngapLog.Errorf("NAS Decode Fail: %+v", err)
					return
				}

				// store eapsuccess, in fact I only store the identifier in eapsuccess
				wagfUe.EAPSuccessID = nasMsg.GmmMessage.SecurityModeCommand.EAPMessage.GetEAPMessage()[1]
				fmt.Println("nasMsg is ", nasMsg.GmmMessage.SecurityModeCommand.EAPMessage.GetEAPMessage())
				fmt.Println("nasMsg is ", nasMsg.GmmMessage.SecurityModeCommand.EAPMessage.GetEAPMessage()[1])
				spew.Config.Indent = "\t"
				nasStr := spew.Sdump(nasMsg)
				ngapLog.Tracef("Get NAS Security Mode Command Message:\n %+v", nasStr)

				// wagfUe.DLCount.AddOne()

				// Send NAS Security Mode Complete Msg
				registrationRequest := gmm_message.GetRegistrationRequest(nasMessage.RegistrationType5GSInitialRegistration,
					*(wagfUe.UEIdentity), nil, wagfUe.GetUESecurityCapability(), wagfUe.Get5GMMCapability(), nil, nil)
				securitymodecmd := gmm_message.GetSecurityModeComplete(registrationRequest)
				if securitymodecmd, err = gmm_message.EncodeNasPduWithSecurity(wagfUe, securitymodecmd, nas.SecurityHeaderTypeIntegrityProtectedAndCipheredWithNew5gNasSecurityContext, true, true); err != nil {
					ngapLog.Errorf("EncodeNasPduWithSecurity: %+v", err)
					return
				}
				ngap_message.SendUplinkNASTransport(amf, wagfUe, securitymodecmd)
				wagfUe.RadiusSession.State = handler.AuthenticationVerify
				return
			} else if wagfUe.RadiusSession.State == handler.AuthenticationVerify {
				_, err := security.NASDecode(wagfUe, nas.SecurityHeaderTypeIntegrityProtectedAndCipheredWithNew5gNasSecurityContext, nasPDU.Value)
				if err != nil {
					ngapLog.Errorf("NAS Decode Fail: %+v", err)
					return
				}
				...
				registrationComplete := nasPacket_message.GetRegistrationComplete(nil)
				// Fixme: the encode
				registrationComplete, err = ngapPacket_message.EncodeNasPduInEnvelopeWithSecurity(wagfUe, registrationComplete, nas.SecurityHeaderTypeIntegrityProtected, true, false)
				if err != nil {
					ngapLog.Errorf("EncodeNasPduWithSecurity: %+v", err)
					return
				}
				ngap_message.SendUplinkNASTransport(amf, wagfUe, registrationComplete)
				return
			}
			...
			wagfUe.TemporaryCachedNASMessage = nasEnv
		}
	}
}
...
func HandlePDUSessionResourceSetupRequest(amf *context.WAGFAMF, message *ngapType.NGAPPDU) {
	...
	for _, ie := range pduSessionResourceSetupRequest.ProtocolIEs.List {
		...
		case ngapType.ProtocolIEIDAMFUENGAPID:
			...
			wagfUe = amf.FindUeByAmfUeNgapID(amfUeNgapID.Value)
		case ngapType.ProtocolIEIDRANUENGAPID:
			ngapLog.Traceln("[NGAP] Decode IE RANUENGAPID")
			ranUeNgapID = ie.Value.RANUENGAPID
			if ranUeNgapID == nil {
	...
	if nasPDU != nil {
		// TODO: Send NAS to UE
		if wagfUe.TCPConnection == nil {
			ngapLog.Error("No IPSec NAS signalling SA for this UE")
			return
		} else {
			// Using a "NAS message envelope" to transport a NAS message
			// over the non-3GPP access between the UE and the wagf
			nasEnv := encapNasMsgToEnvelope(nasPDU)

			if n, err := wagfUe.TCPConnection.Write(nasEnv); err != nil {
	...
	if wagfUe.TemporaryPDUSessionSetupData != nil {
		ngap_message.SendPDUSessionResourceSetupResponse(amf, wagfUe,
			wagfUe.TemporaryPDUSessionSetupData.SetupListSURes,
			wagfUe.TemporaryPDUSessionSetupData.FailedListSURes, nil)
		// for {
		// if len(wagfUe.TemporaryPDUSessionSetupData.UnactivatedPDUSession) != 0 {
		// 	pduSessionID := wagfUe.TemporaryPDUSessionSetupData.UnactivatedPDUSession[0]
		// 	pduSession := wagfUe.PduSessionList[pduSessionID]

		// 	ikeSecurityAssociation := wagfUe.WAGFIKESecurityAssociation
		...
		// 		if _, ok := wagfSelf.ChildSA.Load(uint32(randomUint64)); !ok {
		...
		// 	wagfUe.CreateHalfChildSA(ikeMessage.MessageID, spi, pduSessionID)
		...
		// 	// TSi
		// 	wagfIPAddr := net.ParseIP(wagfSelf.IPSecGatewayAddress)
		...
		// 		0, 65535, wagfIPAddr.To4(), wagfIPAddr.To4())

		// 	// TSr
		// 	ueIPAddr := wagfUe.IPSecInnerIP
		...
		// 	ikePayload.BuildNotifyUP_IP4_ADDRESS(wagfSelf.IPSecGatewayAddress)

		// 	if err := ike_handler.EncryptProcedure(
		// 		wagfUe.WAGFIKESecurityAssociation, ikePayload, ikeMessage); err != nil {
		// 		ngapLog.Errorf("Encrypting IKE message failed: %+v", err)
		// 		wagfUe.TemporaryPDUSessionSetupData.UnactivatedPDUSession =
		// 			wagfUe.TemporaryPDUSessionSetupData.UnactivatedPDUSession[1:]
		// 		cause := buildCause(ngapType.CausePresentTransport,
		// 			ngapType.CauseTransportPresentTransportResourceUnavailable)
		// 		transfer, err := ngap_message.BuildPDUSessionResourceSetupUnsuccessfulTransfer(*cause, nil)
		// 		if err != nil {
		...
		// 			wagfUe.TemporaryPDUSessionSetupData.FailedListSURes,
		// 			pduSessionID, transfer)
		// 		continue
		// 	}
		// 	ike_handler.SendIKEMessageToUE(wagfUe.IKEConnection.Conn, wagfUe.IKEConnection.WAGFAddr,
		// 		wagfUe.IKEConnection.UEAddr, ikeMessage)
		// 	break
		// } else {
		...
		// After all IPsec Child SAs are established, the wagf shall forward to UE via the signalling IPsec SA
		// the PDU Session Establishment Accept message
		fmt.Println("pduSessionEstablishmentAccept: ", pduSessionEstablishmentAccept)
		nasEnv := encapNasMsgToEnvelope(pduSessionEstablishmentAccept)

		nasMsg, err := nasPacket_message.DecodePDUSessionEstablishmentAccept(wagfUe, len(nasEnv), nasEnv)
		if err != nil {
			ngapLog.Errorf("DecodePDUSessionEstablishmentAccept Fail: %+v", err)
		}
		spew.Config.Indent = "\t"
		nasStr := spew.Sdump(nasMsg)
		ngapLog.Trace("Dump DecodePDUSessionEstablishmentAccept:\n", nasStr)
		var pduAddr net.IP
		pduAddr, err = nasPacket_message.GetPDUAddress(nasMsg.GsmMessage.PDUSessionEstablishmentAccept)
		if err != nil {
			ngapLog.Errorf("GetPDUAddress Fail: %+v", err)
		}

		ngapLog.Infof("PDU Address: %s", pduAddr.String())
		// send DHCP ACK to UE
...
func HandlePDUSessionResourceModifyRequest(amf *context.WAGFAMF, message *ngapType.NGAPPDU) {
	...
	// 		if pduSession = wagfUe.FindPDUSession(pduSessionID); pduSession == nil {
	...
	// ngap_message.SendPDUSessionResourceModifyResponse(amf, wagfUe, responseList, failedListModRes, nil)
}
...
func HandlePDUSessionResourceModifyConfirm(amf *context.WAGFAMF, message *ngapType.NGAPPDU) {
	...
	// if rANUENGAPID != nil {
	// 	var ok bool
	// 	ue, ok = wagfSelf.UePoolLoad(rANUENGAPID.Value)
	// 	if !ok {
	// 		ngapLog.Errorf("Unknown local UE NGAP ID. RanUENGAPID: %d", rANUENGAPID.Value)
	// 		return
	// 	}
...
func HandleRANConfigurationUpdateFailure(amf *context.WAGFAMF, message *ngapType.NGAPPDU) {
	...
	// 	wagfSelf.AMFReInitAvailableListStore(amf.SCTPAddr, false)
	// 	time.AfterFunc(time.Duration(waitingTime)*time.Second, func() {
	// 		ngapLog.Infof("Re-send Ran Configuration Update Message when waiting time expired")
	// 		wagfSelf.AMFReInitAvailableListStore(amf.SCTPAddr, true)
