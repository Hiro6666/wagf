...
import (
	...
	"github.com/free5gc/ngap/ngapType" // Fixme: the gmm_message content (redundant!!) is the same as the nasPduUL_message, need to be the same
...
func HandleDownlinkNASTransport(amf *context.WAGFAMF, message *ngapType.NGAPPDU) {
	...
	if nasPDU != nil {
		fmt.Println(wagfUe)
		fmt.Printf("wagfUe.IsSMCRequest: %v\n", wagfUe.IsSMCRequest)
		fmt.Printf("wagfUe.IsRegistrationAccept: %v\n", wagfUe.IsRegistrationAccept)

		if wagfUe.IsSMCRequest {
			// send NAS Security Mode Complete Msg
			securityModeComplete := nasPacket_message.GetSecurityModeComplete(nil)
			securityModeComplete, err := ngapPacket_message.EncodeNasPduInEnvelopeWithSecurity(wagfUe, securityModeComplete, nas.SecurityHeaderTypePlainNas, true, false)
			if err != nil {
				ngapLog.Errorf("EncodeNasPduWithSecurity: %+v", err)
				return
			}
			ngap_message.SendUplinkNASTransport(amf, wagfUe, securityModeComplete)
			return
		}
		if wagfUe.IsRegistrationAccept {
			// send NAS Registration Complete Msg
			registrationComplete := nasPacket_message.GetRegistrationComplete(nil)
			// Fixme: the encode
			registrationComplete, err := ngapPacket_message.EncodeNasPduInEnvelopeWithSecurity(wagfUe, registrationComplete, nas.SecurityHeaderTypePlainNas, true, false)
			if err != nil {
				ngapLog.Errorf("EncodeNasPduWithSecurity: %+v", err)
				return
			}
