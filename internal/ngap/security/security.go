...
func NASEncode(ue *context.WAGFUe, msg *nas.Message, securityContextAvailable bool, newSecurityContext bool) (
...
//  len(nasPDU)|msgSecurityHeader|mac32|sqn|msg
func NASEnvelopeEncode(ue *context.WAGFUe, msg *nas.Message, securityContextAvailable bool, newSecurityContext bool) (
	payload []byte, err error) {
	var sequenceNumber uint8
	if ue == nil {
		err = fmt.Errorf("amfUe is nil")
	...
	if !securityContextAvailable {
		...
		msgSecurityHeader := []byte{msg.SecurityHeader.ProtocolDiscriminator, msg.SecurityHeader.SecurityHeaderType}
		payload = append(msgSecurityHeader, payload[:]...)
		fmt.Println("msgSecurityHeader: ", msgSecurityHeader)
		fmt.Println("payload: ", payload)
		// Increase UL Count
		ue.ULCount.AddOne()
	}
...
func NASDecode(ue *context.WAGFUe, securityHeaderType uint8, payload []byte) (msg *nas.Message, err error) {
	if ue == nil {
		err = fmt.Errorf("amfUe is nil")
		return
	}
	...
	if securityHeaderType == nas.SecurityHeaderTypePlainNas {
		...
		fmt.Println("receivedMac32", receivedMac32)
		...
		// Caculate ul count
		fmt.Println("in security sequence number", sequenceNumber)
		...
		fmt.Println("ue.IntegrityAlg", ue.IntegrityAlg)
		fmt.Println("ue.KnasInt", ue.KnasInt)
		fmt.Println("ue.DLCount.Get()", ue.DLCount.Get())
		fmt.Println("ue kamf", ue.Kamf)
		fmt.Println("security.Bearer3GPP", ue.GetBearerType())
		fmt.Println("security.DirectionUplink", security.DirectionUplink)
