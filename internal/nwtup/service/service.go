...
import (
	...
	"github.com/free5gc/wagf/internal/gre"
	gtpQoSMsg "github.com/free5gc/wagf/internal/gtp/message"
...
// Run bind and listen IPv4 packet connection on wagf NWt interface
// with UP_IP_ADDRESS, catching GRE encapsulated packets and forward
// to N3 interface.
func Run() error {
	// Local IPSec address
	wagfSelf := context.WAGFSelf()
	listenAddr := wagfSelf.IPSecGatewayAddress
	...
	wagfSelf.NWtIPv4PacketConn = ipv4PacketConn
...
func forward(ueInnerIP string, ifIndex int, rawData []byte) {
	...
	for _, childSA := range ue.WAGFChildSecurityAssociation {
