...
func GetResponseAuth(message []byte) []byte {
	...
	catData := append(message, []byte(wagfSelf.RadiusSecret)...)
...
func (EAPAKAPrime *EAPAKAPrime) Type() EAPType { return EAPTypeEAPAKAPrime }

func (EAPAKAPrime *EAPAKAPrime) marshal() ([]byte, error) {
	radiusLog.Debugln("[EAP][Expanded] marshal(): Start marshalling")

	EAPAKAPrimeData := make([]byte, 4)
	EAPAKAPrimeData[0] = EAPAKAPrime.TypeData
	EAPAKAPrimeData[1] = EAPAKAPrime.SubType
	binary.BigEndian.PutUint16(EAPAKAPrimeData[4:8], 0x0000)

	return EAPAKAPrimeData, nil
}

func (EAPAKAPrime *EAPAKAPrime) unmarshal(rawData []byte) error {
	radiusLog.Debugln("[EAP][EAPAKA'] unmarshal(): Start unmarshalling received bytes")
	radiusLog.Tracef("[EAP][EAPAKA'] unmarshal(): Payload length %d bytes", len(rawData))

	if len(rawData) > 0 {
		if len(rawData) < 4 {
			return errors.New("EAPAKAPrime: No sufficient bytes to decode the EAP AKA' type")
		}
		EAPAKAPrime.TypeData = rawData[0]
		EAPAKAPrime.SubType = rawData[1]
	}
