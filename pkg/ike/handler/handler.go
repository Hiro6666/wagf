...
func HandleIKESAINIT(udpConn *net.UDPConn, wagfAddr, ueAddr *net.UDPAddr, message *ike_message.IKEMessage) {
	...
	// For NAT-T
	var ueIsBehindNAT, wagfIsBehindNAT bool
	...
	if majorVersion > 2 {
		...
		SendIKEMessageToUE(udpConn, wagfAddr, ueAddr, responseIKEMessage)
	...
	if len(notifications) != 0 {
		for _, notification := range notifications {
			...
			case ike_message.NAT_DETECTION_DESTINATION_IP:
				...
				// Calculate local NAT_DETECTION_SOURCE_IP hash
				// : sha1(ispi | rspi | wagfip | wagfport)
				localDetectionData := make([]byte, 22)
				binary.BigEndian.PutUint64(localDetectionData[0:8], message.InitiatorSPI)
				binary.BigEndian.PutUint64(localDetectionData[8:16], message.ResponderSPI)
				copy(localDetectionData[16:20], wagfAddr.IP.To4())
				binary.BigEndian.PutUint16(localDetectionData[20:22], uint16(wagfAddr.Port))
	...
	ikeSecurityAssociation := wagfSelf.NewIKESecurityAssociation()
	...
	ikeSecurityAssociation.WAGFIsBehindNAT = wagfIsBehindNAT
	...
	copy(natDetectionSourceIP[16:20], wagfAddr.IP.To4())
	binary.BigEndian.PutUint16(natDetectionSourceIP[20:22], uint16(wagfAddr.Port))
	...
	idPayload.BuildIdentificationResponder(ike_message.ID_FQDN, []byte(wagfSelf.FQDN))
...
func HandleIKEAUTH(udpConn *net.UDPConn, wagfAddr, ueAddr *net.UDPAddr, message *ike_message.IKEMessage) {
	...
	ikeSecurityAssociation, ok := wagfSelf.IKESALoad(localSPI)
	if !ok {
	...
	thisUE := wagfSelf.UELoadbyIDi(initiatorID.IDData)
	...
	p0 := []byte{0x01}
	thisUE.Ktipsec, _ = ueauth.GetKDFValue(thisUE.Kwagf, ueauth.FC_FOR_KTIPSEC_KTNAP_DERIVATION, p0, ueauth.KDFLen(p0))
	...
	responseIKEPayload.BuildIdentificationResponder(ike_message.ID_FQDN, []byte(wagfSelf.FQDN))
	...
	var ueIPAddr, wagfIPAddr net.IP
	if addrRequest {
		// IP addresses (IPSec)
		ueIPAddr = wagfSelf.NewInternalUEIPAddr(thisUE).To4()
		wagfIPAddr = net.ParseIP(wagfSelf.IPSecGatewayAddress).To4()
		...
		responseConfiguration.ConfigurationAttribute.BuildConfigurationAttribute(
			ike_message.INTERNAL_IP4_NETMASK, wagfSelf.Subnet.Mask)
	...
	responseTrafficSelectorResponder.TrafficSelectors.BuildIndividualTrafficSelector(
		ike_message.TS_IPV4_ADDR_RANGE, ike_message.IPProtocolAll, 0, 65535, wagfIPAddr.To4(), wagfIPAddr.To4())
	...
	// After wagf produced its inbound SPI, the field will be overwritten with the SPI.
	...
	// NAT-T concern
	if ikeSecurityAssociation.UEIsBehindNAT || ikeSecurityAssociation.WAGFIsBehindNAT {
		childSecurityAssociationContext.EnableEncapsulate = true
		childSecurityAssociationContext.WAGFPort = wagfAddr.Port
		childSecurityAssociationContext.NATPort = ueAddr.Port
	}

	// Notification(NAS_IP_ADDRESS)
	responseIKEPayload.BuildNotifyNAS_IP4_ADDRESS(wagfSelf.IPSecGatewayAddress)

	// Notification(NSA_TCP_PORT)
	responseIKEPayload.BuildNotifyNAS_TCP_PORT(wagfSelf.TCPPort)
	...
	if err = xfrm.ApplyXFRMRule(false, wagfSelf.XfrmIfaceId, childSecurityAssociationContext); err != nil {
	...
	// After this, wagf will forward NAS with Child SA (IPSec SA)
	thisUE.SignallingIPsecSAEstablished = true

	// Store SA into wagfUE
	thisUE.WAGFIKESecurityAssociation = ikeSecurityAssociation
		...
		for {
			if len(thisUE.TemporaryPDUSessionSetupData.UnactivatedPDUSession) != 0 {
				...
				if err := EncryptProcedure(
					thisUE.WAGFIKESecurityAssociation, ikePayload, ikeMessage); err != nil {
					ikeLog.Errorf("Encrypting IKE message failed: %+v", err)
...
func HandleCREATECHILDSA(udpConn *net.UDPConn, wagfAddr, ueAddr *net.UDPAddr, message *ike_message.IKEMessage) {
	...
	ikeSecurityAssociation, ok := wagfSelf.IKESALoad(responderSPI)
	if !ok {
		ikeLog.Warn("Unrecognized SPI")
		// send INFORMATIONAL type message with INVALID_IKE_SPI Notify payload ( OUTSIDE IKE SA )
	...
	newXfrmiId := wagfSelf.XfrmIfaceId
	...
	if len(thisUE.PduSessionList) > 1 {
		...
		wagfIPAddr := net.ParseIP(wagfSelf.IPSecGatewayAddress).To4()
		wagfIPAddrAndSubnet := net.IPNet{IP: wagfIPAddr, Mask: wagfSelf.Subnet.Mask}
		newXfrmiId += wagfSelf.XfrmIfaceId + wagfSelf.XfrmIfaceIdOffsetForUP
		newXfrmiName := fmt.Sprintf("%s-%d", wagfSelf.XfrmIfaceName, newXfrmiId)

		if linkIPSec, err = xfrm.SetupIPsecXfrmi(newXfrmiName, wagfSelf.XfrmParentIfaceName,
			newXfrmiId, wagfIPAddrAndSubnet); err != nil {
			ikeLog.Errorf("Setup XFRM interface %s fail: %+v", newXfrmiName, err)
			return
		}

		wagfSelf.XfrmIfaces.LoadOrStore(newXfrmiId, linkIPSec)
		childSecurityAssociationContext.XfrmIface = linkIPSec
		wagfSelf.XfrmIfaceIdOffsetForUP++
	} else {
		if linkIPSec, ok := wagfSelf.XfrmIfaces.Load(newXfrmiId); ok {
	...
	for {
		if len(temporaryPDUSessionSetupData.UnactivatedPDUSession) != 0 {
			...
			if err := EncryptProcedure(thisUE.WAGFIKESecurityAssociation, ikePayload, ikeMessage); err != nil {
				ikeLog.Errorf("Encrypting IKE message failed: %+v", err)
				temporaryPDUSessionSetupData.UnactivatedPDUSession = temporaryPDUSessionSetupData.UnactivatedPDUSession[1:]
				cause := ngapType.Cause{
					Present: ngapType.CausePresentTransport,
...
func parseIPAddressInformationToChildSecurityAssociation(
	...
	childSecurityAssociation.LocalPublicIPAddr = net.ParseIP(context.WAGFSelf().IKEBindAddress)
